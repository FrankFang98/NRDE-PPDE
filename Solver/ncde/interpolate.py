...
class linear_interpolate:
    def __init__(self,ts,times,X,lag,T):
        super(linear_interpolate,self).__init__()
        self.times=times
        self.lag=lag
        self.T=T
        self.ts=ts
        self.X=X
    def derivative(self,t):
        lag_X=self.X[:,::self.lag,:]
        lag_step=1/(100/self.lag)
        if int((t+0.00001)//lag_step)>=10:
               left_point=lag_X[:,-1,:]
               right_point=lag_X[:,-1,:]
        else:       
               left_point=lag_X[:,int((t+0.00001)//lag_step),:]
               right_point=lag_X[:,int(((t+0.00001)//lag_step)+1),:]
        
        
        
        return (right_point-left_point)/lag_step
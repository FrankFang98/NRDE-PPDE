...
def rdeintx(logsig, h0,x, func, method='midpoint', adjoint=True, return_sequences=True,odestep=float):
    ...
    cell = _NRDECell(logsig_getter, func,x=x)

    # Set options
    t, options, = set_options(logsig,odestep, return_sequences=return_sequences)
...
def set_options(logsig,odestep, return_sequences=True):
    ...
    if return_sequences:
        t = torch.arange(0, 1.01,step=1/(length-1), dtype=torch.float).to(logsig.device)
        options = {'step_size': odestep}
    else:
        options = {'step_size': odestep}
        t = torch.Tensor([0, 1]).to(logsig.device)
    return t, options
...
class _GetLogsignature:
    ...
    def __init__(self, logsig):
        self.knots = np.arange(0,1.01,step=0.1)
        self.logsig = logsig

    def __getitem__(self, t):
        index = bisect.bisect(self.knots, t) - 1
        return self.logsig[:, index]/0.1
...
class _NRDECell(nn.Module):
    ...
    """
    def __init__(self, logsig_getter, func,x):
        super().__init__()

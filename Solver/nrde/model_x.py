...
from Solver.nrde import rdeintx
...
class NeuralRDE(nn.Module):
    ...
    def forward(self, inputs):
        ...
        assert len(inputs) == 3, "`inputs` must be a 2-tuple containing `(inital_values, logsig)`."
        initial, logsig,x = inputs
        h0 = self.initial_linear(initial)

        # Perform the adjoint operation
        out = rdeintx(
            logsig, h0,x, self.func, method=self.solver, adjoint=self.adjoint, return_sequences=self.return_sequences,odestep=self.odestep
        )
...
class _NRDEFunc(nn.Module):
    ...
    """
    def __init__(self, input_dim, logsig_dim, num_layers, hidden_dim):
        super().__init__()
